...
const uploadSuratKRK = async (req, res) => {
    try {
        ...
        if (req.file == null) {
            return errorResponse(req, res, 400, 'Surat KRK Perlu Diisi');
        }
        ...
        const dataKRK = await Trx_dokumen_instansi.findOne({
            where: {
        ...
        if (dataKRK) {
            return errorResponse(req, res, 400, 'Anda Sudah Pernah Memberikan Surat KRK dengan Detail Permintaan Tersebut.');
        }
